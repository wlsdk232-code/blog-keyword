...
# -*- coding: utf-8 -*-
"""
인블로그 / 티스토리 글별 방문자 통계 자동 점검 (Google Analytics 4)
-------------------------------------------------------------
구글 애널리틱스(GA4) Data API 로 각 블로그의 '글별 방문자수·조회수'를
모든 유입 경로(다음·네이버·구글·직접 등) 합산으로 가져옵니다.

- 블로그별 GA4 속성을 각각 조회 (인블로그/티스토리)
...
# (GA4 속성 ID(숫자), 표시 이름, 블로그 베이스 URL)
GA_PROPERTIES = [
    ("543530488", "인블로그", "https://blog.designpunch.co.kr"),
    ("543587373", "티스토리", "https://designpunch.tistory.com"),
]
...
def fetch_ga(client, property_id, days):
    """GA4(속성 1개=블로그 1개): 페이지경로 → {views, users}. (모든 유입 합산)"""
    ...
    try:
        ...
        for r in resp.rows:
            ...
            cur["views"] += int(r.metric_values[0].value or 0)
            cur["users"] += int(r.metric_values[1].value or 0)
    except Exception as e:
        print("    [경고] GA4 조회 실패(%s): %s" % (property_id, str(e)[:160]))
    return out
...
def run(args):
    ...
    print("■ 인블로그·티스토리 방문자 점검 (GA4, 최근 %d일)" % args.days)
    posts_db = {}
    for pid, label, base in GA_PROPERTIES:
        posts = check_google.collect_posts(base)
        ga = fetch_ga(client, pid, args.days)
        print("  [%s] 글 %d개 수집 / GA4 방문 발생 페이지 %d개" % (label, len(posts), len(ga)))
        for key, info in posts.items():
            ...
            rec.update({"blog": label, "title": title or url, "url": url, "date": date,
...
<body><div class="wrap">
<h1>디자인펀치 방문자 리포트</h1>
<div class="sub">인블로그 · 티스토리 (구글 애널리틱스 GA4 기준 · 전체 유입 합산) · 점검일시 __NOW__</div>
...
let sortKey="users", sortDir=-1, page=1, perPage=25, query="", visOnly=false, blogSel="";
...
function updateCards(){
  ...
  document.getElementById("sp_visited").innerHTML=sparkSvg(seriesFor(blogSel,"visited"),"#15803d");
  document.getElementById("sp_users").innerHTML=sparkSvg(seriesFor(blogSel,"users"),"#b45309");
  document.getElementById("sp_views").innerHTML=sparkSvg(seriesFor(blogSel,"views"),"#7c3aed");
}
function render(){
  updateCards();
  const d=filtered();
  ...
  const rowsHtml=slice.map(function(r){
    ...
    return '<tr><td class="date" data-label="작성일">'+esc(r.date)+'</td><td class="title" data-label="글 제목">'+title+'</td><td class="blog" data-label="블로그">'+esc(r.blog)+'</td><td class="num" data-label="방문자수">'+u+'</td><td class="num" data-label="조회수">'+v+'</td></tr>';
  }).join("");
  tb.innerHTML = rowsHtml || ('<tr><td colspan="5" class="empty">'+(visOnly?'방문이 발생한 글이 아직 없습니다.':'표시할 글이 없습니다.')+'</td></tr>');
...
def generate_html(data, snapshots=None, days=28):
    ...
    present = [lbl for _p, lbl, _b in GA_PROPERTIES if any(r["blog"] == lbl for r in rows)]
    blog_opts = '<option value="">전체 블로그</option>' + "".join(
        '<option value="%s">%s</option>' % (_html.escape(b), _html.escape(b)) for b in present)
...
def main():
    ap = argparse.ArgumentParser(description="인블로그·티스토리 GA4 방문자 점검")
